// Zindigon League | public account bar (sign up / sign in / sign out).
// Same pattern as builds/account.js, adapted to LeagueAuth (auth-shared.js).
// Renders into <div id="accountBar"></div> and injects its own sign-in/
// sign-up modal right after it.
...
function renderAccountBar() {
  ...
  if (LeagueAuth.isSignedIn() && LeagueAuth.hasProfile()) {
    el.innerHTML = `
  ...
  if (LeagueAuth.isSignedIn() && !LeagueAuth.hasProfile()) {
    el.innerHTML = `
      <div class="lp-account">
        <form id="acctProfileForm" class="lp-account-inline-form">
    ...
    document.getElementById("acctProfileForm").addEventListener("submit", async (e) => {
      ...
      if (!username) return;
      try {
        await LeagueAuth.createProfile(username);
        renderAccountBar();
        if (typeof onAccountChange === "function") onAccountChange();
      } catch (err) {
...
function injectAccountModal() {
  ...
  bar.insertAdjacentHTML("afterend", `
    <div class="lp-modal-overlay" id="acctModalOverlay" hidden>
      <div class="lp-modal">
        <button class="lp-modal-close" id="acctModalClose" type="button" aria-label="Close">&times;</button>
        ...
        <form id="acctForm">
          ...
          <p class="lp-account-error" id="acctError" hidden></p>
          <p class="lp-account-info" id="acctInfo" hidden></p>
          <button type="submit" class="btn btn-primary" id="acctSubmitBtn">Sign In</button>
        </form>
        <p class="lp-account-switch">
  ...
  document.getElementById("acctForm").addEventListener("submit", async (e) => {
    ...
    try {
      if (acctMode === "signup") {
        const { needsEmailConfirmation } = await LeagueAuth.signUp(email, password);
        ...
        await LeagueAuth.signInWithPassword(email, password);
      }
      closeAccountModal();
      renderAccountBar();
      if (typeof onAccountChange === "function") onAccountChange();
...
async function initAccountBar() {
  ...
  if (!container) return;
  injectAccountModal();
  await LeagueAuth.init();
  renderAccountBar();
  LeagueAuth.onChange(() => renderAccountBar());
